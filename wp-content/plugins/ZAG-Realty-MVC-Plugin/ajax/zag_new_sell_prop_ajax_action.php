...
function zag_new_sell_prop_ajax_action() {
    ...
    if (0 == $current_user->ID || !check_ajax_referer('new_sell_nonce', 'coming_new_sell_nonce', false)) {
...
// может понадобиться дополнительная проверка/очистка имени файла
                    ...
                    if (true) {
                        ...
                        switch ($filetype['ext']) {
                            case 'png':
                                ...
                                $new_file = preg_replace("/\.png$/", ".webp", $new_file);
                                imagewebp($img, $new_file);
                    ...
                    $img_editor = wp_get_image_editor($new_file);
...
// Generate the metadata for the attachment, and update the database record.
                    ...
                    $room = sanitize_text_field($request['select'][$number]);
                    add_post_meta($attach_id, 'room', $room);

                    $room_sq = sanitize_text_field($request['roomSQ'][$number]);

                    add_post_meta($attach_id, 'roomSQ', $room_sq);
